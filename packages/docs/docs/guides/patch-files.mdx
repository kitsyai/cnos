...
:::note
`--cnos-patch` is a **runtime flag**. It is passed to your application process, not to the `cnos` CLI. CNOS runtimes read the flag (or the `CNOS_PATCH_FILE` env var) when they initialize at application startup. You cannot use `--cnos-patch` with CLI authoring commands like `cnos value set` or `cnos build`.
:::
