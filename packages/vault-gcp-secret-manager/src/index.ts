...
interface GcpAccessSecretVersionResponse {
...
export interface GcpSecretManagerClient {
  accessSecretVersion(request: { name: string }): Promise<[GcpAccessSecretVersionResponse, ...unknown[]]>;
  listSecrets?(request: { parent: string }): Promise<[GcpSecret[], ...unknown[]]>;
  getProjectId?(): Promise<string>;
  close?(): Promise<void>;
}

export interface GcpSecretManagerVaultConfig {
  projectId?: string;
  location?: string;
  version?: string;
  endpoint?: string;
...
export interface GcpSecretManagerVaultProviderOptions {
  client?: GcpSecretManagerClient | ((vaultId: string, definition: VaultDefinition) => GcpSecretManagerClient);
}
...
function readConfig(definition: VaultDefinition): GcpSecretManagerVaultConfig {
  const config = isRecord(definition.auth?.config) ? definition.auth.config : {};
  const vaultConfig: GcpSecretManagerVaultConfig = {};
...
class GcpSecretManagerVaultProvider implements RemoteSecretVaultProvider {
  private authenticated = false;
  private readonly config: GcpSecretManagerVaultConfig;
  private readonly client: GcpSecretManagerClient;

  constructor(
    readonly vaultId: string,
    readonly definition: VaultDefinition,
    options: GcpSecretManagerVaultProviderOptions = {},
  ) {
  ...
  async authenticate(authConfig: VaultAuthConfig): Promise<void> {
    if (authConfig.method !== 'iam' && authConfig.method !== 'environment') {
      throw new Error(
        `Vault "${this.vaultId}" uses ${GCP_SECRET_MANAGER_VAULT_PROVIDER} and requires iam authentication.`,
      );
    }
  ...
  private async resolveProjectId(): Promise<string> {
    ...
    throw new Error(
      `Vault "${this.vaultId}" requires auth.config.projectId when Google ADC cannot infer a project ID.`,
    );
  }
  ...
  async set(ref: string, value: string): Promise<void> {
    void ref;
    void value;
    throw new Error(`Vault "${this.vaultId}" is Google Secret Manager-backed and read-only.`);
  }
  ...
  async healthCheck(): Promise<{ ok: boolean; message?: string }> {
    try {
      ...
      return { ok: false, message: error instanceof Error ? error.message : 'Google Secret Manager health check failed.' };
    }
  }
}

/**
 * Creates a compiled-in CNOS provider factory for Google Secret Manager.
 *
 * Authentication is delegated to Google's official client library, so production
 * runtimes should use Application Default Credentials or an attached service account.
 */
export function createGcpSecretManagerVaultProvider(
  options: GcpSecretManagerVaultProviderOptions = {},
): SecretVaultProviderFactory {
  return {
    provider: GCP_SECRET_MANAGER_VAULT_PROVIDER,
    create(vaultId, definition) {
      return new GcpSecretManagerVaultProvider(vaultId, definition, options);
    },
  };
}
