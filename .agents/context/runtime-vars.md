...
- **Required prefetch keys** are checked against the whole overlay after EVERY prefetch outcome (ingested / not-modified / no-head / rejected), not just thrown errors: unresolvable ⇒ `ready()` / `StartVars` fails. Carve-out: a missing transport module is warned and non-fatal. `refreshVar` on a required key rejects on a transport failure or a validation-`rejected` revision, but NOT on a no-head.
- **Watcher dispatch is idempotent**: an exact `(revision, generation)` replay wakes nobody; a watcher registered inside a callback is not visited by the pass already running.
